...
// assumes cadet alphas in standard positions (i.e. not AHoV).
const uint16_t PROGMEM and_combo[]                  = { UM_TOP, UM_RTAC, COMBO_END };
const uint16_t PROGMEM identity_combo[]             = { UM_RTAC, UM_LTAC, COMBO_END };
const uint16_t PROGMEM less_than_or_equal_combo[]   = { UM_LEFT, UM_RGHT, COMBO_END };
const uint16_t PROGMEM partial_differential_combo[] = { UM_RGHT, UM_LR, COMBO_END };
#   endif
#endif
...
#   if defined(CADET_ALPHAS)
    COMBO(and_combo,                  UM_AND),
    COMBO(identity_combo,             UM_IDEN),
    COMBO(less_than_or_equal_combo,   UM_LEQL),
    COMBO(partial_differential_combo, UM_PDIF),
#   endif
#endif
};
